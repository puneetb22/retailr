def generate_shop_bill_template(invoice_data, pdf_buffer):
    """
    Generate a PDF invoice based on the shop_bill.pdf template with exact layout matching
    ...
    try:
        ...
        # Create main outer table for the entire invoice structure
        # This will be a single table with properly nested tables for each section
        
        # First, we'll create each section as a separate table, then combine them
        ...
        print(f"Error generating shop bill template: {e}")
        import traceback
        traceback.print_exc()
        return False